...
export async function POST(req: Request) {
  try {
    ...
    if (!refresh) {
      return NextResponse.json({ success: false, message: 'Refresh token requerido' }, { status: 400 });
    }

    const decoded = verifyRefreshToken(refresh);
    if (!decoded?.sub) {
      return NextResponse.json({ success: false, message: 'Refresh inválido' }, { status: 401 });
    }
